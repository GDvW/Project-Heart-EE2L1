...
Simulation allows to verify that the localization algorithm works properly the exact location of the sources can be specified when generating the audio files that simulate the heart beat. Both MVDR and MUSIC were tested. 
...
After experimenting the with the depth needed as explained in \ref{depth_analysis}, the optimal depth turned out to be 0.15 meters. As it can be seen both algorithms perform satisfactory. It was expected that MUSIC will have sharper peaks than MVDR because it is a higher resolution algorithm.
...
\begin{figure} [H]
    \centering
    \includegraphics[width=0.6\textwidth]{docs/assignments/Final report/figures/figures/Localization/Localization results/Calibrated_MT---_bin0_z0.02_Fs48000_Q2_M6_v_sound340_d0.1_resolution0.001_modemusic.png}
    \caption{Power plot at a depth of 0.15m using the MUSIC algorithm with 2 sources using the phantom model.}  
    \label{fig:music_3D_2sources_phantom}
\end{figure}

\begin{figure} [H]
    \centering
    \includegraphics[width=0.6\textwidth]{docs/assignments/Final report/figures/figures/Localization/Localization results/Calibrated_MT---_bin0_z0.02_Fs48000_Q2_M6_v_sound340_d0.1_resolution0.001_modemvdr.png}
    \caption{Power plot at a depth of 0.15m using the MVDR algorithm with 2 sources using the phantom model.}  
...
\end{figure}
It can be inferred that both methods give similar results. The only difference is that MUSIC has sharper peaks (thus darker colours further away from the valves) while the MVDR gives a smeared out result, which is expected. It can also been seen that the estimated sources are in the top left of the plot. This is likely due to the microphone array not being placed right above the heart.
