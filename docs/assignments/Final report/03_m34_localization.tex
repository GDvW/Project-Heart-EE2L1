...
To achieve this, in addition to the angle in horizontal plane, the elevation angle is also needed. Also, the far field assumption does not hold. The valves (sources) are close to the microphone array, which leads to each microphone seeing the source from a significantly different angle.
...
For the algorithms it is also essential that only signals are used which contain useful information, which is why the signals first have to be segmented into S1 peaks and S2 peaks before they are analyzed by the algorithms.
...
\end{itemize}
Simulation and phantom model can be used with the sources playing white noise or a heartbeat. The recordings of a persons heart only include heartbeat signals. In case heartbeat signals were used, the S1 and S2 peaks were first segmented using the preprocessing system. In case white noise was used as a signal, this was not done, as it wouldn't have any effect and would be meaningless.
...
The results using the S2 peak sounds are shown here, the results using S1 sounds and white noise have been given in \ref{assignment_9.2}.
\begin{figure} [H]
    \centering
...
In the plots it becomes clear that the MVDR algorithm localizes the sources more accurately. The MUSIC algorithm comes to a depth of 0.07m, which is smaller than the set depth. This could be due to the simulation method not aligning with the method that MUSIC uses to localize the sources.
...
\end{figure}
What can be observed is that the depth at which the most power is observed is extremely small. Nonetheless, both algorithms seem to localize the sources with less than a centimeter of error. Although MUSIC is considered as a higher resolution algorithm, it turned out to have similar accuracy to the MVDR. Both algorithms seem to estimate the sources with a comparable offset. This could mean that the true sources or the microphone array were not perfectly centered when conducting the measurements.

\subsection{Human Recordings}
Next, the algorithms were applied on the recordings of a persons heart. It is not known were the exact locations of the heart valves were, so this was meant as a demonstration. Also the propagation speed of the signal is difficult to be estimated in the tissue, but by varying it there was no significant difference in the results.

\begin{figure} [H]
    \centering
    \includegraphics[width=0.6\textwidth]{docs/assignments/Final report/figures/figures/Localization/NEW/RealLocalizationResults/S2-9489d50d-9398-4cfe-9c2f-0d0c9ee67536---_bin0_z0.001_Fs48000_Q2_M6_v_sound340_d0.1_resolution0.001_modemusic_neighbourhood3.png}
    \caption{Power plot of a human recording of S2 sounds at a depth of 0.001m using the MUSIC algorithm.}  
    \label{fig: music_3D_2sources_person}
\end{figure}

\begin{figure} [H]
    \centering
    \includegraphics[width=0.6\textwidth]{docs/assignments/Final report/figures/figures/Localization/NEW/RealLocalizationResults/S2-9489d50d-9398-4cfe-9c2f-0d0c9ee67536---_bin0_z0.001_Fs48000_Q2_M6_v_sound340_d0.1_resolution0.001_modemvdr_neighbourhood3.png}
    \caption{Power plot of a human recording of S2 sounds at a depth of 0.001m using the MVDR algorithm.}  
    \label{fig: mvdr_3D_2sources_person}
\end{figure}
It can be inferred that both methods give similar results. The only difference is that MUSIC has sharper peaks (thus darker colors further away from the valves) while the MVDR gives a more smeared out result, which is expected. It can also be observed that one source is estimated directly under microphone 4. This could be due to the fact that the recording of that microphone was notably greater in amplitude than the other recordings. A reason for this could be the placement of the microphone array, such that microphone 4 was able to detect the most sound.
...
The simulations show that the method can recover the correct source locations, and that MUSIC gives sharper peaks than MVDR. However, MUSIC did not correctly estimate the location of the sources.
With the phantom model, both methods performed to satisfaction, as the estimated locations were close to the expected holes in the foam. 
...
Changing the assumed propagation speed did not strongly change the results in any of the test cases.
